...
def registration(request, chat_id, username):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            staff = Staff(username=username, telegram_id=chat_id, cities="", name=name, surname=surname, pin=pin, phone=phone, instagram=instagram, user=user, avatar='avatars/ava_F68lBxi_xbswx2G.jpeg')
            staff.save()
            login(request, user)
