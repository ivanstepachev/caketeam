    ...
    if request.method == "POST":
        ...
        if "message" in data:
            ...
            if 'username' in data["message"]["chat"]:
                ...
                text = '''Привет! Для того, чтобы начать пользоваться сервисом и зарегистрироваться на платформе, Вам требуется создать в своём профиле Telegram "Имя пользователя". Для этого перейдите в настройки профиля, нажмите кнопку "Изм." в верхнем углу и в открывшемся меню добавьте Имя пользователя! 
                
                ❗ Пока пользуетесь нашей платформой Имя пользователя изменять нельзя! 
                После создания Имени пользователя нажмите /start, чтобы присутпить к регистрации.'''
